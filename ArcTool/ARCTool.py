...
class rarc_node_class:
    ...
    #2 bytes unknown
    numFileEntries = 0  # how many files belong to this node?
    ...
    while dstplace < data_size:
        ...
        if quiet == False:
            ...
            if int(calcpercent) > percent:
                ...
                sys.stdout.flush()
                percent = int(calcpercent)
    if quiet == False:
        print("\nWriting output")
    output.write(bytes(dst))
...
def getString(pos, f):
    ...
    while True:
        ...
        data.extend(char)
    f.seek(t)
    # use latin-1 to preserve 1:1 byte mapping
...
def processNode(node, h, f):
    ...
    for i in range(0, node.numFileEntries):
        ...
        if currfile.id == 0xFFFF:  # file is a subdir
            if currname != "." and currname != "..":  # do not go to "." and ".."
                ...
                while size > 0:
                    if quiet == False:
                        ...
                        if calcpercent > percent:
                            ...
                            sys.stdout.flush()
                            percent = calcpercent
                    # note: this writes whatever remains, same logic as original
...
def get_u8_name(i, g, node):
    ...
    while True:
        ...
        if t == b"\0" or t == b"":
            break
        retval.append(t)
    return b"".join(retval).decode("latin-1", errors="ignore")
                ...
                while size > 0:
                    if quiet == False:
                        ...
                        if calcpercent > percent:
                            ...
                            sys.stdout.flush()
                            percent = calcpercent
                    # fixed obvious handle typo from original: read from i
...
def main():
    ...
    for file in args:
        ...
        elif the_type == b"RARC":
            if quiet == False:
                print("RARC archive")
            unrarc(f, of)
        elif the_type == b"U\xAA8-":
            if quiet == False:
                print("U8 archive")
            unu8(f, of)
        else:
